...
struct Node
{
	int pri, val, pa, ls, rs, tag, siz;
	
	Node() : pri(randint(1, 1e9)), val(0), pa(0), ls(0), rs(0), siz(0) {}
	Node(int _val) : Node()
	{
		val = _val;
		siz = 1;
...
int kth(int x, int k)
{
	...
	if (k == lsiz + 1) return t[x].val;
	pushdown(x);
	if (lsiz >= k) return kth(t[x].ls, k);
	return kth(t[x].rs, k - lsiz - 1);
}

int build(const vi &a, int l, int r)
{
	if (l > r) return 0;
	int mid = (l + r) >> 1;
	int u = t.size();
	t.eb(a[mid]);
	t[u].ls = build(a, l, mid - 1);
	t[u].rs = build(a, mid + 1, r);
	pushup(u);
	return u;
}

int build(const vi &a)
{
	t.resize(1);
	t.reserve(a.size() + 1);
...
{
	pii res = split(x, t[y].val);
	return merge(merge(res.fi, y), res.se);
}
...
signed main()
{
	...
	while (m--)
	{
		if (read() == 1) wtb(kth(root, read()));
		else
		{
			int x = read();
			pii res = split(root, x + 1);
			int small = res.fi;
			res = split(res.se, x * 2 + 1);
			int middle = res.fi;
			int large = res.se;
			add(large, -x);
			...
			function<void(int)> dfs = [&](int u)
			{
				...
				dfs(t[u].rs);
				t[u] = Node(t[u].val - x);
				small = insert(small, u);
			};
			
			dfs(middle);
			
			root = merge(small, large);
		}
	}
